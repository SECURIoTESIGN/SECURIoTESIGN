...
These security best practice guidelines aim to ensure that applications developed using the Java language are free from, or resilient to, code injection attacks.
...
Check [LDAP Injection Prevention Cheat Sheet](https://cheatsheetseries.owasp.org/cheatsheets/LDAP_Injection_Prevention_Cheat_Sheet.html) has been created.
...
As there many NoSQL database system and each one use an API for call, it's important: 
 * To ensure that user input received and used to build the API call expression does not contain any character that have a special meaning in the target API syntax. 
